  ...
  const init = async () => {
    ...
    if (modeType === "user")
      ...
      if (response.data.status === 200) {
        responseHandler({ ...response.data.data, type: "client" });
      }
    }
  };
            ...
            <Box direction="row" gap="small">
              ...
              <Box>
                ...
                <Text style={{ fontSize: "13px" }}>{`${new Date(
                  ...
                  new Date(item.workerFbDate).getMonth() + 1
                }-${new Date(item.workerFbDate).getFullYear()} | ${new Date(
                  item.workerFbDate
                ).getHours()}:${new Date(
                  item.workerFbDate
                ).getMinutes()}`}</Text>
      ...
      >
        <Box
          ...
          style={{
            ...
            width: width < 600 ? "300px" : width / 2,
            height: width < 600 ? "auto" : height / 1.5,
            alignSelf: "center",
            borderRadius: "10px",
            padding: "50px",
          }}
